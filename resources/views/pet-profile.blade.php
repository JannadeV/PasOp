...
<div id="indicators-carousel" class="relative w-full" data-carousel="static">
    <div class="relative h-56 overflow-hidden rounded-lg md:h-96">
        ...
        @foreach($huisdier->dierfotos as $foto)
        @if($foto = $huisdier->dierfotos[0])
        <div class="duration-700 ease-in-out" data-carousel-item="active">
            <img src={{ asset($foto->path) }} class="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="...">
        </div>
        @else
    ...
    <div class="absolute z-30 flex -translate-x-1/2 space-x-3 rtl:space-x-reverse bottom-5 left-1/2">
        @for($i=0; $i<count($huisdier->dierfotos); $i++ )
        @if($i == 0)
